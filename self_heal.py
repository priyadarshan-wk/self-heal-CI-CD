...
def analyze_with_openai(error_log, affected_code):
    ...
    try:
        response = openai.Completion.create(
            ...
            prompt=f"The following is an error log from a CI/CD pipeline. Please analyze it and suggest a fix for the affected line(s) of code. The error log is as follows:\n\n{error_log}\n\nHere is the affected code snippet (in context):\n\n{affected_code}\n\nPlease provide the smallest code change necessary to fix the issue, either by modifying the existing line or adding new lines.",
            temperature=0.3,
            max_tokens=150  # Limit to a small fix
        )
